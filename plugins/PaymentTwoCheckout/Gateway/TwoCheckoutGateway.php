...
class TwoCheckoutGateway extends \Modules\Order\Gateways\BaseGateway
{
    ...
    public function handlePurchaseData(\Modules\Order\Models\Payment $payment)
    {
        $request = \request();
        $twocheckout_args = array();
        $twocheckout_args['sid'] = $this->getOption('twocheckout_account_number');
        $twocheckout_args['paypal_direct'] = 'Y';
        $twocheckout_args['cart_order_id'] = $payment->order_id;
        $twocheckout_args['merchant_order_id'] = $payment->order_id;
        $twocheckout_args['total'] = (float)$payment->amount;
        $twocheckout_args['return_url'] = $this->getCancelUrl() . '?c=' . $payment->id;
        $twocheckout_args['x_receipt_link_url'] = $this->getReturnUrl() . '?c=' . $payment->id;
        ...
        $twocheckout_args['email'] = auth()->user()->email;
